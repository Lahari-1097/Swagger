...
const options = {
  swaggerDefinition: {
    info: {
      title: 'Personal Budget API @Lahari Prathapagiri',
      version: '2.0.0',
 ...
 *     responses:
 *       200:
 *         description: Object 'shoes' objects with prices
 */
...
app.patch('/prices/:name', validatePriceRequest, (req, res) => {
  ...
  const itemToUpdate = prices.shoes.find((item) => item.name === itemName);
  if (!itemToUpdate) {
...
app.put('/prices/:name', validatePriceRequest, (req, res) => {
  ...
  const itemIndex = prices.shoes.findIndex((item) => item.name === itemName);
  if (itemIndex === -1) {
    return res.status(404).json({ error: 'Item not found' });
  }
  ...
  prices.shoes[itemIndex] = newItem;
  res.json(newItem);
});
...
app.listen(port, () => {
  console.log(`API server at http://157.230.184.208:${port}`);
});